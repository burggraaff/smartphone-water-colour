...
Linearise using standard sRGB settings.
...
for folder_main in folders:
    for tup in walk(folder_main):
        ...
        # Linearise JPEG data
        water_jpeg = sRGB_inverse(water_jpeg, normalization=1.0)
        sky_jpeg = sRGB_inverse(sky_jpeg, normalization=1.0)
        card_jpeg = sRGB_inverse(card_jpeg, normalization=1.0)
        ...
        hc.histogram_jpeg(water_all, sky_all, card_all, saveto=data_path/"statistics_jpeg_linear.pdf", normalisation=1.)
        ...
        saveto = data_path.with_name(data_path.stem + "_jpeg_linear.csv")
