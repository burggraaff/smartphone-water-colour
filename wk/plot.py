...
def correlation_plot_bands(x, y, datalabel="R_rs", errlabel=None, quantity="$R_{rs}$", xlabel="", ylabel="", saveto=None):
    ...
    if errlabel is not None:
        GB_err_label, GR_err_label, RB_err_label = [f"{datalabel} ({c})" for c in hc.bandratio_labels]
    ...
    correlation_plot_simple(x_RB, y_RB, xerr=x_err_GR, yerr=y_err_GR, ax=axs[2], xlabel=f"{quantity} R/B\n{xlabel}", ylabel=f"{quantity} R/B\n{ylabel}", equal_aspect=True)
